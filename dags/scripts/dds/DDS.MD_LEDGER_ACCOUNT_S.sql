...
INSERT INTO DDS.MD_LEDGER_ACCOUNT_S
(
    chapter
    ,chapter_name
    ,section_number
...
DROP TABLE IF EXISTS REJECTED_DATA.MD_LEDGER_ACCOUNT_S;
CREATE TABLE REJECTED_DATA.MD_LEDGER_ACCOUNT_S AS 
SELECT 
    r.*
FROM RAW.MD_EXCHANGE_RATE_D r
LEFT JOIN (SELECT DISTINCT id FROM MD_LEDGER_ACCOUNT_S_TEMP) t
    ON r.id::INT = t.id 
WHERE t.id IS NULL;
...
SELECT 
    '{{dag.dag_id}}'
    ,'DDS.MD_LEDGER_ACCOUNT_S upserted ' || COUNT(*)::VARCHAR || ' rows'
    ,NOW()
FROM MD_LEDGER_ACCOUNT_S_TEMP;
